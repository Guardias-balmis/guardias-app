...
test("guardarPreferencias y misPreferencias hacen round-trip; solo afecta al propio residente", () => {
  const deps = makeDeps();
  const session = loggedIn(deps);
  const prefs = { maxGuardias: 5, preferDobles: true, diasPreferidos: ["V"], diasEvitar: ["L"], rotDe: null, rotHasta: null, vacDe: null, vacHasta: null, notas: "ninguna" };
  ...
  assert.deepEqual(r2.prefs.diasPreferidos, ["V"]);
