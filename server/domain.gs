...
//
// Tres riesgos, ninguno nuevo salvo el tercero:
...
//      `aviso`.
//   3. Concentración por NIVEL (nuevo, no estaba en la propuesta original): más de 2 residentes
//      del mismo nivel exacto (R1, R2, R3 o R4 — NUNCA grupo Mayor/Pequeño, NUNCA cohorte)
//      ausentes a la vez, aunque el grupo en conjunto siga teniendo gente para cubrir. Siempre
//      `aviso`, nunca bloquea: sigue siendo factible con el resto del grupo.
//
// Cuidado al implementar sobre esto: este tercer eje agrupa por `nivel` (R1-R4, derivado de
// fecha, cambia en el aniversario de cada residente), NO por `cohorte` (año calendario de
// `fechaInicio`, la que usa INV-6 para su propio "máx. 2 simultáneos" en rotación). Reutilizar
// por error la agrupación de INV-6 aquí mediría otra cosa sin que ningún test lo note, porque
// ambas reglas se VEN igual ("máx. 2 a la vez"). `CLAUDE.md` ya marca `cohorte` y `nivel` como
// "distintas — don't conflate them".

  const { compareISO, addDays, addMonths } = Calendar;
...
function previewBloqueoRisk(nuevo, { residentes, bloqueosActivos, today }) {
  ...
  // 3. Concentración por nivel (nuevo) — siempre aviso, nunca bloquea.
  ...
  return { previewWindowEnd, previewBloqueoRisk };
})();
...
// ── API pública ──
var Domain = Object.assign({}, Calendar, Residents, Tally, Absences, BlockPreview, Imaginaria, Accumulate, Thirdpost, Equity, Validate, Responsible, Cuadrante, Projection);
