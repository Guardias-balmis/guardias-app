 ...
 *
 * Anular importa tanto como registrar: el registro se hace DESPUÉS de la incidencia, deprisa y por
 * teléfono, así que apuntar al residente equivocado es el error normal, no el raro. Sin este botón
 * la única forma de corregirlo era entrar al Sheet a mano, que es justo lo que la app existe para
 * evitar. La fila no se borra —la tabla es append-only, se reinserta con activo=false— igual que
...
function Imaginaria({ api, residentes, showToast, puedoRegistrar }) {
  ...
  // Las coberturas YA registradas de esa incidencia (grupo + día), con su id: lo que hace anulable
  // una fila concreta. La cola no las lleva — solo la fecha de la última de cada uno.
  ...
  const cargar = async () => {
    ...
    setBusy(false);
    // `r.coberturas || []` y no `r.coberturas` a secas: hasta que el backend nuevo esté desplegado
    // (server-lib.gs se pega a mano), un /exec viejo responde sin ese campo — y la lista de abajo
    // se quedaría en `undefined`, tumbando la tarjeta entera de Inicio.
  ...
  const anular = async (cobertura) => {
    ...
    setBusy(false);
    // `cargar()` solo tras un cambio real, no en los dos casos: tras un fallo, releer daría la
