...
| 8 | **Generador de cuadrante con IA (Gemma vía Gemini API) en el servidor**, en sustitución de la pantalla «prompt portátil» | 🟡 **En curso (2026-08-30)**, decisión V-43. Encargo directo del autor: la generación deja de ser «monta un prompt, pégalo en un asistente y trae el JSON» y pasa a una acción de servidor (`generarCuadranteIA`) que llama a Gemma con la clave guardada en Script Properties, valida la respuesta con el validador de siempre y reintenta hasta 3 veces devolviéndole las violaciones concretas. Supera la premisa del ADR-001 sobre «sin API de terceros» (ver V-43: el motivo era que no había dónde guardar un secreto; ahora lo hay y sigue sin tocar el navegador). Detalle, checklist de requisitos y comprobación de consistencia: sección «Fase 8» más abajo |
...
## Fase 8 · Generador de cuadrante con IA (decisión V-43)
...
|---|---|---|---|
| Permiso | V-43(g): `requireCicloPermiso` / `puedeMoverCiclo` | R2/R3 y §8.3, mismo mecanismo | ✅ |
| Juez de la propuesta | V-43(d): `validateMonth`+`validateThirdPost`, solo `error` bloquea | R8 y §8.3 | ✅ |
| Reintentos | V-43(e): máx. 3, con las violaciones concretas | R9, §8.4 paso 3 | ✅ |
| Fallo final | V-43(e): `REVISION_MANUAL`, ni una fila escrita | R10 | ✅ |
| Escritura | V-43(f): `monthReplacementPlan` en `v2/domain/apply.js` | R11/R14, §8.4 paso 1 | ✅ |
| Secreto y modelo | V-43(b): `GEMINI_API_KEY` en Script Properties, `GEMINI_MODEL` configurable | R7/R12, §8.5 | ✅ |
| Entidad nueva | §2 `Generacion` (mes, anio, modelo, intentos, resultado, violaciones) | §8.3 tercer punto | ✅ |
| Qué se borra | V-43(h): `Generator.jsx` + pestaña | R4 | ✅ |
| Contrapartida | V-43, «contrapartida registrada»: se pierde el prompt portátil | §8.5 primer riesgo | ✅ |

No queda ningún punto de `spec.md` §V-43 sin fila en este plan, ni ninguna fila de este plan sin
