...
| INV-8 | 3P: cubrir L→D antes de repetir día (por voluntario); dif ≤ 1 entre voluntarios al cierre; prioridad a días con R1 de mochila | año | error (repetición, no-voluntario); **aviso** (equidad al cierre —V-14—, prioridad mochila) | ⚠️ **implementado y con 19 tests, pero SIN invocador**: `validateThirdPost` no lo llama nadie (ni `marcarValidado` ni ninguna pantalla) y falta la tabla de voluntarios de 3P, sin la cual marcaría todo 3P como no-voluntario |
...
- [~] `v2/domain/thirdpost.js` — `validateThirdPost`: INV-8 (rotación, equidad, mochila), 19 tests. **Sin invocador**: está en el bundle y en `deps.domain`, pero no lo llama ni `marcarValidado` ni ninguna pantalla, así que las cuatro reglas del 3.º puesto no se comprueban en producción. Cablearlo exige antes una tabla de voluntarios de 3P (con la lista vacía marcaría TODO 3P como no-voluntario) y decidir la severidad de INV-8a/8b, hoy `error` contra la lista cerrada de V-14
- [x] `v2/domain/equity.js` — `validateResidencyYearClose`: INV-3 + INV-4 (cierre de año), 15 tests
- [ ] INV-12 (coherencia festivo) + `calendar.bridgesOfMonth()` — cierra S-4 (puentes derivados, no dato libre). Bloqueado por un dato que no existe: **no hay tabla de festivos**, y de ella cuelgan además el eje `puentesLibres` de INV-3 (hoy compara ceros) y la sección de festivos del prompt del generador (que hoy le pide a la IA distinguir GF/GP sin darle una sola fecha festiva, justo lo que S-4 prohíbe)
