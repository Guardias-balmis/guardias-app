...
- **La equidad nunca bloquea (decisión V-14).** Toda violación de equidad —INV-3 en sus dos cierres, INV-8 en su diferencia de 3.º puestos— es `aviso`, jamás `error`, porque una regla de equidad que bloquea deja al servicio sin cuadrante cuando ya no queda nadie que sepa desbloquearla (el criterio rector es que la app sobreviva sin administrador). No "arregles" esto subiendo una a `error`: la contrapartida acordada es que la UI pide confirmación explícita («Validar de todas formas») antes de validar un cuadrante que las incumple, usando `cuadrante.js:equityWarnings`/`EQUITY_INVARIANTS` — nunca reconociendo el mensaje por su texto.
