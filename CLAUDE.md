...
- **C-1 (lookahead):** any monthly `dobletes` computation that gets summed across months (Sheets projection, equity accumulation) must feed `tally()` the target month's assignments **plus** the first ~2 days of the next month, or a Friday-31→Sunday doblete silently vanishes — this is the exact bug already fixed once (S-5). The residency-year close's month contribution is one of these: `router.js:closeViolations` and `client/lib/closes.js` pass the first two days of the next month along with the validated month (2026-09-07 — before that, the resident closing a year on Friday the 30th lost that doblete while an earlier-closing cohort-mate, measured from full history, kept it). `projection.js`'s per-tab rows deliberately do *not* do this lookahead (documented, matches the legacy `.xlsm`) — don't "fix" that without re-reading its header comment.
...
Storage (`sheets-schema.js` + `sheets-store.js`): 9 tables are append-only and UUID-keyed, never rewritten row-by-row; `readLatest` projects "current state" as last-row-wins per key. All writes serialize through one Apps Script script lock (a per-user lock wouldn't serialize anything since "Execute as Me" runs everything under one identity). Projected/derived sheets (monthly tab + "Resumen") are republished via a crash-safe shadow-swap (`_tmp_` sheet → delete old → rename) — this is only for projected output, never for the append-only data tables. **Fixed production bug worth remembering:** Google Sheets silently auto-converts an ISO date string cell to its internal Date type on write; `sheets-schema.js`'s `"date"` column type works around it by apostrophe-prefixing on serialize and handling both a real `Date` (via *local*-timezone getters, matching `Code.gs`'s Europe/Madrid formatting — deliberately not the UTC convention used elsewhere in the domain) and the apostrophe-prefixed string on deserialize. Since 2026-09-04 **every plain-text column gets the same apostrophe**: `setValues` also runs any cell starting with `=` as a formula and auto-converts anything that looks like a date or boolean (a `notas` of "15/7" came back as a JS `Date` string), so `deserialize` strips one leading apostrophe for all text and the test fakes never need to emulate Sheets' stripping. `rowsToRecords` also drops rows whose every cell is empty (a hand-cleared row, never a record).

**Generador con IA (`generarCuadranteIA`, V-45/V-47):** el modelo propone y el validador dispone, siempre. Desde V-47 tiene dos modos y el defecto es `completar`: las guardias que YA están en la rejilla (G/GF/GP/3P) son inamovibles —los residentes apuntan en su fila las que ya tienen comprometidas antes de generar—, van al prompt como «ya fijadas», el validador juzga el mes RESULTANTE (fijadas + propuesta) y `apply.js:monthCompletionPlan` nunca emite una fila de borrado. `reemplazar` es lo de V-45 (`monthReplacementPlan`, el mes entero se sustituye). No hay marca «fijada» en ninguna tabla: una guardia fijada es una fila de `asignaciones` como cualquier otra — no inventes un campo para distinguirlas.
...
No bundler, no build step. `index.html` loads React/ReactDOM/GIS from CDN tags plus one `<script type="module" src="client/loader.js">`. `loader.js` exists because Babel Standalone's automatic `<script type="text/babel">` DOM-scanner was found to silently drop one of the app's JSX files with no error — so `loader.js` instead fetches (in parallel), transpiles, and `import()`s each file from a `Blob` URL sequentially, in a fixed order (`components.jsx` → screens → `App.jsx` last, since `App.jsx` consumes `window.Screens.*`). Since 2026-09-04 the transpiled output is cached in `localStorage` keyed by the SHA-256 of each source file, and Babel Standalone (~2,3 MB) is injected by `loader.js` only when some file misses the cache — it is no longer a `<script>` in `index.html`; `BABEL_URL`/`CACHE_PREFIX` in `loader.js` are the single place to bump its version. `loader.js` also prefetches the login nonce (`auth.js:prefetchNonce`) before any screen exists.
